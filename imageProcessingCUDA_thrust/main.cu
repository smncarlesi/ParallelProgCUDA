...
__host__ void buildImage(thrust::host_vector<unsigned char> &finalDatasArr, int width, int height, int channels,
                         cv::Mat *finalImage) {
...
__host__ void assignDatas(thrust::host_vector<unsigned char> &imageDatas, int width, int height, int channels,
                          cv::Mat *image) {
    int arrI = 0;
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
...
__host__ void printRGBValues(thrust::host_vector<unsigned char> &imageDatas, int totalSize, int channels) {
    int pixelID = 0;
...
__device__ void printArr(thrust::device_vector<unsigned char> &d_imageDatas) {
    for (int i = 0; i < d_imageDatas.size(); ++i) {
        printf("d_imageDatas[%d]: %d\n", i, static_cast<int>(d_imageDatas[i]));
    }
}
...
int main() {
    ...
    cudaSetDevice(0);
    thrust::host_vector<unsigned char> imageDatas(totalSize);
    thrust::host_vector<unsigned char> outputImageDatas(totalSize);
    ...
    /*<--------------------------------DEVICE MEM ALLOC and COPY-------------------------------->*/
    auto beginCUDATime = std::chrono::high_resolution_clock::now(); /*CUDA CHRONO START*/

    thrust::device_vector<unsigned char> d_imageDatas = imageDatas;

    thrust::device_vector<unsigned char> d_outputImageDatas(totalSize);

    auto endCUDATime = std::chrono::high_resolution_clock::now(); /*CUDA CHRONO END*/
    std::chrono::duration<double, std::milli> ms_double_dev_mem = endCUDATime - beginCUDATime;
    ...
    thrust::device_vector<unsigned char>::iterator begin = d_imageDatas.begin();
    thrust::device_vector<unsigned char>::iterator outBegin = d_outputImageDatas.begin();
    unsigned char *d_imageDatasPtr = thrust::raw_pointer_cast(&(*begin));
    unsigned char *d_outputImageDatasPtr = thrust::raw_pointer_cast(&(*outBegin));

    kernelConvolution<<<gridDim, blockDim>>>(d_imageDatasPtr, d_outputImageDatasPtr);
    ...
    thrust::copy(d_outputImageDatas.begin(), d_outputImageDatas.end(), outputImageDatas.begin());
    ...
    printf("TOTAL TIMING: %f ms\n",
           ms_double_dev_mem.count() + ms_double_kernel.count() + ms_double_dev_host.count() + ms_double_host.count());
