...
__global__ void kernelConvolution(unsigned char *d_imageDatas, unsigned char *d_outputImageDatas, int startRow, int numRows) {
    ...
    for (int k = 0; k < CHANNELS; ++k) {
        ...
        __syncthreads();
        if (globalY >= startRow + MASK_RADIUS_Y && globalY < startRow + numRows + MASK_RADIUS_Y && globalX < IMG_WIDTH) {
            ...
            if (y < IMG_HEIGHT && x < IMG_WIDTH) {
                d_outputImageDatas[((globalY - startRow) * IMG_WIDTH + x) * CHANNELS + k] = (unsigned char) devCLamp(sum);
            }
        }
    }
}
